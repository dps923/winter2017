...
class Model {
    ...
    lazy var frc_example: NSFetchedResultsController<Example> = {
        ...
        let frc = NSFetchedResultsController<Example>(fetchRequest: fetchRequest, managedObjectContext: CDStack.shared.viewContext, sectionNameKeyPath: nil, cacheName: nil)
        return frc
    }()
    ...
    init() {
        ...
        if isFirstLaunch {
            if hasStarterData {
                ...
                StoreInitializer.create(CDStack.shared)
            }
        }
