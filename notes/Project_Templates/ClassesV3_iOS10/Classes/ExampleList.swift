...
class ExampleList: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // CDStack is initialized in the background, can't be sure it is ready just yet
        if (CDStack.shared.isReady) {
            performFetch()
        } else {
            NotificationCenter.default.addObserver(self, selector: #selector(ExampleList.performFetch), name: .onCDStackReady, object: nil)
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "toExampleDetail" {
            ...
            let item = frc?.object(at: self.tableView.indexPathForSelectedRow!)
